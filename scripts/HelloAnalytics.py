...
def run_test_cases_report(client: BetaAnalyticsDataClient, property_id):
  ...
  response: RunReportResponse = client.run_report(request)
...
def check_test_case_report(client: BetaAnalyticsDataClient, property_id):
  request = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="customEvent:user_upi"), Dimension(name="customEvent:user_testing_group"), Dimension(name="customEvent:question_number"), Dimension(name="customEvent:current_test_case")],
  ...
  print("Writing check_test_case events to file");
  # Print to file
  with open("check_test_case_report.txt", "w") as f:
    for dimension_header in response.dimension_headers:
      dimension_header.name = dimension_header.name.replace("customEvent:", "")
      f.write(f"{dimension_header.name}\t")
    for metric_header in response.metric_headers:
  ...
  print("Added check_test_case events to file")
